...
def train_one_epoch(model, dataloader, criterion, optimizer, device):
    ...
    for batch in tqdm(dataloader, leave=False, desc="Entrenando"):
...
def objective_hyperopt(params):
    ...
    alpha = params.get("dice_weight", 0.5)
    ...
    run_name = f"hyperopt_{timestamp}"
    ...
    train_sampler = RandomBatchGeoSampler(train_ds, IMG_SIZE, batch_size, TRAIN_SAMPLE_SIZE)
    val_sampler = RandomBatchGeoSampler(val_ds, IMG_SIZE, batch_size, VAL_SAMPLE_SIZE)
    test_sampler = GridGeoSampler(test_ds, IMG_SIZE, stride=IMG_SIZE//2)
    ...
    if fixed_sample is None:
        raise ValueError("❌ No se encontró muestra fija para test")

    log_fixed_sample(fixed_sample, DEVICE, scaled_means, scaled_stds)

    optimizer = optim.AdamW(model.parameters(), lr=lr)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=sched_patience, factor=sched_factor, mode="max")

    val_iou = train_model(
        model, train_loader, val_loader, criterion, optimizer, scheduler, metric,
        DEVICE, EPOCHS, fixed_sample, "hyperopt_trial.ckpt", writer
    )

    load_checkpoint(model, "hyperopt_trial.ckpt", DEVICE)
    test_loss, test_iou = test_model_center_crop(model, test_loader, criterion, metric, DEVICE, center_margin=IMG_SIZE // 8)
    ...
    return {
        "loss": -val_iou,  # Hyperopt minimiza, por eso usamos -IoU
        "status": STATUS_OK,
        "test_iou": test_iou,
        "val_iou": val_iou,
        "test_loss": test_loss,
...
if __name__ == "__main__":
    ...
    space = {
    "lr": hp.loguniform("lr", np.log(1e-4), np.log(5e-3)),  # FPN tiende a beneficiarse de LR ligeramente más altos
    "loss": hp.choice("loss", ["ce+dice", "focal"]),
    "dice_weight": hp.uniform("dice_weight", 0.6, 0.9),     # enfatiza Dice
    "batch_size": hp.choice("batch_size", [2, 3]),        
    "sched_patience": hp.quniform("sched_patience", 3, 6, 1),
    "sched_factor": hp.uniform("sched_factor", 0.1, 0.3),
    }   
    ...
    # Mostrar resumen ordenado por Test IoU
    results = []
    for t in trials.results:
        if t["status"] == STATUS_OK:
            results.append({
                "Val IoU": round(t["val_iou"], 4),
                "Test IoU": round(t["test_iou"], 4),
                "Test Loss": round(t["test_loss"], 4),
                **t["params"]
            })

    df_hyperopt = pd.DataFrame(results).sort_values("Test IoU", ascending=False).reset_index(drop=True)
    print("\n📊 Resultados ordenados por Test IoU:")
    print(df_hyperopt.to_string(index=False))
...
for idx, t in enumerate(trials.trials):
    ...
    if res["status"] == STATUS_OK:
        results.append({
            "Trial": idx,
            "Val IoU": round(res["val_iou"], 4),
            "Test IoU": round(res["test_iou"], 4),
            "Test Loss": round(res["test_loss"], 4),
            "LR": round(res["params"]["lr"], 6),
            "Loss": res["params"]["loss"],
            "Dice α": round(res["params"].get("dice_weight", 0), 4),
            "Batch": res["params"]["batch_size"],
            "Patience": int(res["params"]["sched_patience"]),
            "Sched Factor": round(res["params"]["sched_factor"], 4),
        })
